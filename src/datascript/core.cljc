...
(def ^{:arglists '([db tx-data] [db tx-data tx-meta])} with
  ...
  #?(:cljd (fn ([db tx-data] (conn/with db tx-data))
               ([db tx-data tx-meta] (conn/with db tx-data tx-meta)))
     :default conn/with))

(def ^{:arglists '([db tx-data]) :tag DB} db-with
  "Applies transaction to an immutable db value, returning new immutable db value. Same as `(:db-after (with db tx-data))`."
  #?(:cljd #(conn/db-with %1 %2) :default conn/db-with))
...
(defn ^#?(:cljd Datom? :default Datom) find-datom
...
(def ^{:arglists '([conn tx-data] [conn tx-data tx-meta])} transact!
  ...
  Usage:
      ...
      (transact! conn [[:db/add  -1 :name   \"Oleg\"]
                       [:db/add 296 :friend -1]])"
  #?(:cljd (fn ([conn tx-data] (conn/transact! conn tx-data))
               ([conn tx-data tx-meta] (conn/transact! conn tx-data tx-meta)))
     :default conn/transact!))
...
(def ^{:arglists '([conn db] [conn db tx-meta])} reset-conn!
  ...
  #?(:cljd (fn ([conn db] (conn/reset-conn! conn db))
               ([conn db tx-meta] (conn/reset-conn! conn db tx-meta)))
     :default conn/reset-conn!))

(def ^{:arglists '([conn schema])} reset-schema!
  #?(:cljd #(conn/reset-schema! %1 %2) :default conn/reset-schema!))
...
(def ^{:arglists '([conn callback] [conn key callback])} listen!
  ...
  #?(:cljd (fn ([conn callback] (conn/listen! conn callback))
               ([conn key callback] (conn/listen! conn key callback)))
     :default conn/listen!))

(def ^{:arglists '([conn key])} unlisten!
  "Removes registered listener from connection. See also [[listen!]]."
  #?(:cljd #(conn/unlisten! %1 %2) :default conn/unlisten!))
